...
def test_parse_mdl_atom_table(sample_rxn_text):
    mol_blocks = run_rdt.split_rxn_to_mols(sample_rxn_text)
    atoms = run_rdt.parse_mdl_atom_table(mol_blocks[0])
    expected = [
        ("O", 1), ("C", 2), ("C", 3), ("O", 4), ("C", 5),
...
def test_parse_mdl_atom_table_mol02(sample_rxn_text):
    mol_blocks = run_rdt.split_rxn_to_mols(sample_rxn_text)
    atoms = run_rdt.parse_mdl_atom_table(mol_blocks[1])
    expected = [
...
def test_parse_mdl_atom_table_mol03(sample_rxn_text):
    mol_blocks = run_rdt.split_rxn_to_mols(sample_rxn_text)
    atoms = run_rdt.parse_mdl_atom_table(mol_blocks[2])
    assert len(atoms) == 20
    assert atoms[0] == ("O", 1)
    assert atoms[1] == ("C", 2)
...
def test_parse_inchi_atom_order(sample_rxn_text):
    mol_blocks = run_rdt.split_rxn_to_mols(sample_rxn_text)
    inchi_text = run_rdt.obabel_to_inchi(mol_blocks[0])
    order = run_rdt.parse_inchi_atom_order(inchi_text)
    assert order == [2, 5, 3, 1, 4, 8, 9, 10, 6, 7]


@pytest.mark.integration
def test_parse_inchi_atom_order_mol02(sample_rxn_text):
    mol_blocks = run_rdt.split_rxn_to_mols(sample_rxn_text)
    inchi_text = run_rdt.obabel_to_inchi(mol_blocks[1])
    order = run_rdt.parse_inchi_atom_order(inchi_text)
    assert order == [3, 5, 2, 4, 1, 8, 9, 10, 6, 7]


@pytest.mark.integration
def test_parse_inchi_atom_order_mol03(sample_rxn_text):
    mol_blocks = run_rdt.split_rxn_to_mols(sample_rxn_text)
    inchi_text = run_rdt.obabel_to_inchi(mol_blocks[2])
    order = run_rdt.parse_inchi_atom_order(inchi_text)
...
def test_parse_inchi_atom_order_multicomponent():
    inchi_text = (
        "InChI=1S/2H2O/h2*1H2\n"
        "AuxInfo=1/1/N:3,4;1,2/rA:3H1H2H1H2/rB;s1;s2;s1;s2/rC:;;;\n"
    )
    order = run_rdt.parse_inchi_atom_order(inchi_text)
